#' @title Extract Branch-Specific Standard Gene Regulatory Networks (sGRNs)
#'
#' @description
#' This function extracts branch-specific standard gene regulatory networks (sGRNs) from the complete cell-type-specific sGRNs.
...
#'
#' @param interest_cell_type_sGRN_all_TGTF_pairs The output of function get_sGRN_by_TFBS_pwm_by_JASPAR2024.
...
#' \item Second level: For each cell type, a list of branch-specific sGRNs
#' \item Each branch sGRN is a data frame with:
#' \itemize{
#' \item Rows: Target genes (TGs) present in the branch
#' \item Columns: Transcription factors (TFs) present in the branch
#' \item Values: Normalized PWM match scores from the complete sGRN
#' }
#' }
#' The branch sGRNs preserve the regulatory strength values from the complete network but are filtered to include only the TFs and TGs that are expressed/active in each branch.
#'
#' @details
...
#' \item Populates the matrices with regulatory strength values from the complete sGRN
...
#' )
#' }
get_branch_sGRN = function(interest_cell_type_sGRN_all_TGTF_pairs = interest_cell_type_sGRN_all_TGTF_pairs,
                           interest_cell_type_group = interest_cell_type_group,
                           ncores = 1)
{
  ### Start.
  t_start = base::Sys.time()
  message("Run: Extracting sGRN for each branch from every cell type's sGRN ", t_start, ".")
  ...
  get_branch_sGRN_process = function(cell_type, interest_cell_type_sGRN_all_TGTF_pairs, interest_cell_type_group)
  {
    branch_sGRN = base::list()
    for (n in 1:base::length(interest_cell_type_group[[cell_type]][["n_min"]])) {
      ...
      branch_sGRN[[n]] = base::as.data.frame(base::matrix(0, nrow = base::length(branch_TGs), ncol = base::length(branch_TFs)))
      base::rownames(branch_sGRN[[n]]) = branch_TGs
      base::colnames(branch_sGRN[[n]]) = branch_TFs
      for (TG in branch_TGs) {
        for (TF in branch_TFs) {
          if (TG %in% base::rownames(interest_cell_type_sGRN_all_TGTF_pairs[[cell_type]]) &&
              TF %in% base::colnames(interest_cell_type_sGRN_all_TGTF_pairs[[cell_type]])) {
            branch_sGRN[[n]][TG, TF] = interest_cell_type_sGRN_all_TGTF_pairs[[cell_type]][TG, TF]
          }
        }
      }
    }
  ...
  interest_cell_type_branch_sGRN = parallel::mclapply(
    X = base::names(interest_cell_type_sGRN_all_TGTF_pairs),
    FUN = function(cell_type) {
      message("Preparing to extract sGRN for each branch in cell type ", cell_type, ".")
      get_branch_sGRN_process(
        cell_type = cell_type,
        interest_cell_type_sGRN_all_TGTF_pairs = interest_cell_type_sGRN_all_TGTF_pairs[cell_type],
        interest_cell_type_group = interest_cell_type_group[cell_type]
      )
    },
    mc.cores = ncores
  )
  base::names(interest_cell_type_branch_sGRN) = base::names(interest_cell_type_sGRN_all_TGTF_pairs)

  ### End.
  base::setwd(original_dir)
  message("The current working directory has been switched to: ", base::getwd(), ".")
  t_end = base::Sys.time()
  message("Finish: Extracting sGRN for each branch from every cell type's sGRN ", t_end, ".")
  message("Running time: ")
  base::print(t_end - t_start)
  return(interest_cell_type_branch_sGRN)
}
