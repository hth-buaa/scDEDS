#' @title Set Lower Bounds for Model Parameters in Gene Regulatory Network Prediction
#'
#' @description
#' This function establishes lower bound constraints for the parameters of the gene regulatory network prediction model.
...
#'
#' @returns A nested list structure containing lower bound values for all model parameters.
#'
#' @details
#' The function performs the following operations:
#' \enumerate{
#' \item Creates the directory structure for storing model training results
#' \item Sets biologically plausible lower bounds for all parameter types
...
#' \itemize{
#' \item Lower bounds are set based on biological constraints and numerical stability requirements
...
#' )
#' }
set_init_params_lower = function(interest_cell_type_branch_dataset_spilt = interest_cell_type_branch_dataset_spilt,
                                 ncores = 1)
{
  ### Start.
  t_start = base::Sys.time()
  message("Run: Setting parameter lower bounds for all branche ", t_start, ".")
  ...
  set_init_params_process = function(TGs, TFs, Ks)
  {
    ...
    for (TG in TGs) {init_params[["r"]][["r2_TG"]][[TG]] = 0.370506}
    init_params[["r"]][["r2_TF"]] = base::list()
    for (TF in TFs) {init_params[["r"]][["r2_TF"]][[TF]] = 0.370506}
    ...
    for (TG in TGs) {
      init_params[["alpha"]][["alpha1_TG_K-1"]][[TG]] = rep(0.1, base::length(Ks))
      base::names(init_params[["alpha"]][["alpha1_TG_K-1"]][[TG]]) = Ks - 1
    }
    init_params[["alpha"]][["alpha1_TF_K-1"]] = base::list()
    for (TF in TFs) {
      init_params[["alpha"]][["alpha1_TF_K-1"]][[TF]] = rep(0.1, base::length(Ks))
    ...
    for (TG in TGs) {
      init_params[["alpha"]][["alpha2_TG_K-1"]][[TG]] = rep(0.1, base::length(Ks))
      base::names(init_params[["alpha"]][["alpha2_TG_K-1"]][[TG]]) = Ks - 1
    }
    init_params[["alpha"]][["alpha2_TF_K-1"]] = base::list()
    for (TF in TFs) {
      init_params[["alpha"]][["alpha2_TF_K-1"]][[TF]] = rep(0.1, base::length(Ks))
    ...
    for (TG in TGs) {init_params[["s"]][["s1_TG"]][[TG]] = 0.1}
    init_params[["s"]][["s1_TF"]] = base::list()
    for (TF in TFs) {init_params[["s"]][["s1_TF"]][[TF]] = 0.1}
    message("Setting s2 in s.")
    init_params[["s"]][["s2_TG"]] = base::list()
    for (TG in TGs) {init_params[["s"]][["s2_TG"]][[TG]] = 0.1}
    init_params[["s"]][["s2_TF"]] = base::list()
    for (TF in TFs) {init_params[["s"]][["s2_TF"]][[TF]] = 0.1}

    message("Setting u.")
    init_params[["u"]] = base::list()
    message("Setting u1 in u.")
    init_params[["u"]][["u11_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u11_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u21_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u21_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u311_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u311_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u321_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u321_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u331_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u331_TG"]][[TG]] = 0.1}
    message("Setting u2 in u.")
    init_params[["u"]][["u12_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u12_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u22_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u22_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u312_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u312_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u322_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u322_TG"]][[TG]] = 0.1}
    init_params[["u"]][["u332_TG"]] = base::list()
    for (TG in TGs) {init_params[["u"]][["u332_TG"]][[TG]] = 0.1}
    ...
    for (TG in TGs) {init_params[["v"]][["v1_TG"]][[TG]] = 0.1}
    message("Setting v2 in v.")
    init_params[["v"]][["v2_TG"]] = base::list()
    for (TG in TGs) {init_params[["v"]][["v2_TG"]][[TG]] = 0.1}
    message("Setting v3 in v.")
    init_params[["v"]][["v3_TG"]] = base::list()
    for (TG in TGs) {init_params[["v"]][["v3_TG"]][[TG]] = 0.1}
  ...
  for (cell_type in base::names(interest_cell_type_branch_dataset_spilt)) {
    message("Setting initial parameter lower bounds for the data of cell type ", cell_type, ".")
    ...
    interest_cell_type_branch_init_params[[cell_type]] = parallel::mclapply(
      ...
      FUN = function(n) {
        message("Setting initial parameter lower bounds for the data of branch ", n, " of cell type ", cell_type, ".")
  ...
  message("The current working directory has been switched to: ", base::getwd(), ".")
  t_end = base::Sys.time()
  message("Finish: Setting parameter lower bounds for all branche ", t_end, ".")
